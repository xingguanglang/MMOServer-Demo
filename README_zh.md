...
- [x] **阶段 3** — 10 Hz 状态同步广播 + ebiten 客户端(客户端插值)
- [x] **阶段 4** — 压测机器人(1–2k 虚拟玩家)+ 性能数据 + AOI 开/关对比
- [x] **阶段 5a** — 分布式拆分:网关 + 场景两进程,经 gRPC 双向流通信
- [x] **阶段 6** — Docker Compose、GitHub Actions CI、中英 README + 设计文档

计划外额外做的:

- [x] 按距离分级更新(LOD)—— 近处走 AOI 高频 + 其余走低频全局流
- [x] 网页管理台 + HTTP API —— 生成/移动/压测/清空/开窗口/实时改帧率/切 AOI/指标
- [x] 上帝视角(观战)客户端

可选的后续:

- [ ] **阶段 5b** — Redis(热)+ MySQL(持久)数据层;定时 + 下线落库
- [ ] **阶段 5c** — 轻量战斗服(gRPC 跨服调用演示)
